...
function production {
	sudo sed -i "s|/var/www/maintenace|/var/www/html|g" /etc/nginx/conf.d/production.conf
	sudo sed -i "s|/var/www/html|/var/www/maintenance|g" /etc/nginx/conf.d/maintenance.conf
	sudo $serv1 restart
	sudo $serv2 restart
}

function maintenance {
	sudo sed -i "s|/var/www/html|/var/www/maintenance|g" /etc/nginx/conf.d/production.conf
	sudo sed -i "s|/var/www/maintenance|/var/www/html|g" /etc/nginx/conf.d/maintenance.conf
        sudo $serv1 restart
        sudo $serv2 restart
}
